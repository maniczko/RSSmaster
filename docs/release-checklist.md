...
| `npm run check:perf:browser` | warm browser route-ready p95 for `/read/inbox`, `/read/saved`, and `/sources` with warning/fail thresholds | cold boot, API business correctness, long-session memory profile | `output/playwright/browser-perf-smoke.json` |
| `npm run check:perf:workspace` | workspace ranking/stories/briefing API p95 with warning/fail thresholds | browser render time, semantic ranking quality, full API contract | `output/playwright/workspace-perf-smoke.json` |
