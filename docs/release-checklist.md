...
| `npm run check:auth` | isolated browser smoke for local auth: first-account register, protected app open, logout, login, invalid password feedback, 401 guard, and standardized `artifact` schema | canonical cold boot, password manager behavior, multi-account migration, long-lived session expiry | `output/playwright/auth-smoke/auth-smoke.json`, `output/playwright/auth-smoke/auth-smoke.png` |
...
| `npm run check:magazines` | isolated browser smoke for the `/magazines` product surface: visible magazine heading, premium magazine shell, seeded issue archive, issue detail, `/magazines?issue=<id>` deep link restore, article snapshot, issue-level preflight, digest-based issue controls, no console/page errors, no horizontal overflow, and standardized artifact schema | independent magazine storage model, scheduler, live Kindle acceptance, spoken screen-reader behavior | `output/playwright/magazines-smoke.json`, `output/playwright/magazines-smoke.png` |
...
| `npm run check:reader:interaction` | browser smoke for the premium reader loop: open cleaned article, visible `Wyślij na Kindle` article action with configuration feedback, `Przeczytaj + dalej`, mobile target sizing, no horizontal overflow, and undo readiness; uses an isolated runtime when the current runtime is auth-guarded | full corpus extraction quality, live SMTP/Kindle acceptance, screen-reader spoken behavior, canonical cold boot | `output/playwright/reader-interaction-smoke.json`, `output/playwright/reader-interaction-smoke.png` |
...
Manual live delivery sign-off lives in `docs/runbooks/live-delivery-signoff.md`. Copy `docs/runbooks/live-delivery-evidence-template.md` into ignored `output/live-delivery/` before recording live SMTP and Kindle acceptance evidence.
