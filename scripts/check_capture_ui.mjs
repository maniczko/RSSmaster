...
async function openReaderNotes(page) {
  const hideButton = await findVisibleButtonByExactText(page, "Ukryj notatki");
  if (hideButton) {
    return;
  }

  let noteButton = await findVisibleButtonByExactText(page, "Notatki i tagi");
  if (!noteButton) {
    const openArticleButton = await findVisibleButtonByExactText(page, "Czytaj");
    ...
    noteButton = await findVisibleButtonByExactText(page, "Notatki i tagi");
  }

  assert(noteButton, "Reader did not render the notes toggle after opening the captured article.");
...
async function main() {
  ...
  try {
    ...
    const titleInput = page.getByPlaceholder("Nadpisz tytul tylko, jesli potrzebujesz");
    const noteInput = page.getByPlaceholder("Dlaczego warto do tego wrocic");
    ...
    await page.waitForSelector("text=Artykul jest juz w RSSmasterze", { timeout: 20000 });
    ...
    await page.getByRole("button", { name: "Otworz zapisany artykul" }).click();
    ...
    await openReaderNotes(page);
    await page.waitForSelector(`text=${initialNote}`, { timeout: 20000 });
